...
func LoadHandler(appService *core_services_app.AppService) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		logger := appService.LoggerService.With(
			slog.String("label", "handlers.control_calculator.LoadHandler"),
			slog.String("request_id", middleware.GetReqID(r.Context())),
		)

		var request requests_control_calculator.LoadRequest
		...
		controlCalculatorModel, err := appService.ControlCalculatorService.Load(request)

		if err != nil {
			message := "failed to load controlCalculatorModel"
